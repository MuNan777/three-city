<template>
  <div id="bigScreen">
    ...
    <div class="main">
      <div class="left">
        <div class="cityEvent" v-for="(item, key) in dataInfo">
          <h3>
            <span>{{ item.name }}</span>
          </h3>
          <h1>
...
import { warnType } from '../Scene/three/mesh/warnSprite'
...
export interface DataInfo {
  iot: { name: string, number: number, unit: string },
  event: { name: string, number: number, unit: string },
  power: { name: string, number: number, unit: string },
  test: { name: string, number: number, unit: string },
}
